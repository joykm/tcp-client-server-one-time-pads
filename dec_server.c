...
* 4. in the child processes, decrypt client messages, and send plain text in return
...
// ensure only dec_client can connect to server
int connectionGranted(int connectionSocket, char* arg0) {
  ...
  // ensure it is dec_client connecting
  if (idBuffer[0] != 'd') {
  ...
  // send binary "true" if dec_client is attempting to connect
...
// recieve data and key, decrypt data, and send cypher
void decryption(int connectionSocket, char* arg0) {
  ...
  while (eofFound == 0) {
    ...
    // use two pointers to iterate the cipher and key simulaneously
    for (lPtr = 0; lPtr < midpointCeil; lPtr++) {
      cipherChar = inputBuffer[lPtr];
      ...
      adjust characters to 0 to 26 index, subtract key ansii decimal
      from the cipher ansii decimal, and find remainder when divided 
      by 27 */
      remainder = ((cipherChar - 65) - (keyChar - 65)) % 27;
      ...
      outputBuffer[lPtr] = plainTextChar;             
      rPtr++;
    }
  ...
  while(1){
    ...
    switch(childPid) {
      ...
      case 0:
        ...
        // if dec_client is requesting connection, perform decryption
        if (connectionGranted(connectionSocket, argv[0])) {
          decryption(connectionSocket, argv[0]);
        }       
